...
// From https://andrewkelley.me/post/string-matching-comptime-perfect-hashing-zig.html
...
pub fn perfectHash(comptime strs: []const []const u8) type {
    ...
    var ops_buf: [14]Op = undefined;

    var seed: u32 = 0x3214567;
    const plan = have_a_plan: {
        var index_i: usize = 0;
        const try_seed_count = 100;
        const try_index_count = 100;
    ...
    if (true) {
        var buf = [_]u8{0} ** 80;
        var seed_str = try std.fmt.bufPrint(&buf, "{d}", .{seed});
        @compileError("Use seed " ++ seed_str);
    }
